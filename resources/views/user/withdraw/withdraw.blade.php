					...
					<div class="user-profile-details">
						<div class="order-history">
							<div class="header-area">
								<h4 class="title" >
									{{ $langg->lang336 }}
									<a class="mybtn1" href="{{route('user-wwt-index',$storename)}}"> <i class="fas fa-arrow-left"></i> {{ $langg->lang337 }}</a>
								</h4>
							</div>

                            {{-- id="userform" --}}
                                                <div class="gocover" style="background: url({{ asset('assets/images/'.$gs->loader) }}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
                         <form  class="form-horizontal" action="{{route('user-wwt-store',$storename)}}" method="POST" enctype="multipart/form-data">

                                                    {{ csrf_field() }}

                                                    @include('includes.admin.form-both') 
                                <div class="form-group">
                                    <label class="control-label col-sm-12" for="name">{{ $langg->lang355 }} {{ App\Models\Product::vendorConvertPrice($storename,Auth::user()->affilate_income) }}</label>
                                </div>
