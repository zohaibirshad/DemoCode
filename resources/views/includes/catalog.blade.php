        <div class="col-lg-3 col-md-6">
          <div class="left-area">
            ...
            <div class="body-area">
              <form id="catalogForm" action="{{ route('front.category', [$storename,Request::route('category'), Request::route('subcategory'), Request::route('childcategory')]) }}" method="GET">
                ...
                <ul class="filter-list">
                  ...
                  @if($element->storename === $storename)
                    <li>
                    <div class="content">
                        <a href="{{route('front.category', [$storename,$element->slug] )}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i> {{$element->name}}</a>
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            @foreach ($cat->subs as $key => $subelement)
                            <div class="sub-content open">
                              <a href="{{route('front.category', [$storename,$cat->slug, $subelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>{{$subelement->name}}</a>
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                  <a href="{{route('front.category', [$storename,$cat->slug, $subcat->slug, $childcat->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i> {{$childcat->name}}</a>
                                </div>
                                @endforeach
                              @endif
                            </div>
            ...
            @if ((!empty($cat) && !empty(json_decode($cat->attributes, true))) || (!empty($subcat) && !empty(json_decode($subcat->attributes, true))) || (!empty($childcat) && !empty(json_decode($childcat->attributes, true))))
              ...
              <div class="tags-area">
                ...
                <div class="body-area">
                  <form id="attrForm" action="{{route('front.category', [$storename,Request::route('category'), Request::route('subcategory'), Request::route('childcategory')])}}" method="post">
                    <ul class="filter">
                ...
                <div class="header-area">
                  ...
                  <div class="body-area">
                    <ul class="taglist">
                      ...
                      @if(!empty($tag))
                      <li>
                        <a class="{{ isset($tags) ? ($tag == $tags ? 'active' : '') : ''}}" href="{{ route('front.tag',[$storename,$tag]) }}">
                            {{ $tag }}
                        </a>
                      </li>
                      @endif
