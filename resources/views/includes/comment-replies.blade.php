...
<div class="write-comment-area">
  <form id="comment-form" action="{{ route('product.comment',$storename) }}" method="POST">
    {{csrf_field()}}
...
@foreach($productt->comments()->orderBy('created_at','desc')->get() as $comment)
  <li>
      ...
      <div class="right-area">
        ...
        <div class="comment-footer">
          ...
          @if(Auth::guard('web')->user()->id == $comment->user->id)
            ...
            <a href="javascript:;" data-href="{{ route('product.comment.delete',[$storename,$comment->id]) }}" class="comment-link comment-delete mr-2"><i class="fas fa-trash"></i>{{ $langg->lang112 }}</a>
          @endif
          </div>
        </div>
      </div>
    ...
    <div class="replay-area edit-area">
      <form class="update" action="{{ route('product.comment.edit',[$storename,$comment->id]) }}" method="POST">
        {{csrf_field()}}
...
@if($comment->replies)
  @foreach($comment->replies as $reply)
      ...
      <div class="right-area">
        ...
        <div class="comment-footer">
          ...
          @if(Auth::guard('web')->user()->id == $reply->user->id)
            <a href="javascript:;" class="comment-link edit mr-2"><i class="fas fa-edit "></i>{{ $langg->lang111 }}</a>
            <a href="javascript:;" data-href="{{ route('product.reply.delete',[$storename,$reply->id]) }}" class="comment-link reply-delete mr-2"><i class="fas fa-trash"></i>{{ $langg->lang112 }}</a>
          @endif
          </div>
        </div>
      </div>
    ...
    <div class="replay-area edit-area">
      <form class="update" action="{{ route('product.reply.edit',[$storename,$reply->id]) }}" method="POST">
        {{csrf_field()}}
...
@endif
    ...
    <div class="replay-area reply-reply-area">
      <form class="reply-form" action="{{ route('product.reply',[$storename,$comment->id]) }}" method="POST">
        {{ csrf_field() }}
