                ...
                <div class="row">
                  <div class="col-lg-12">
                    <div class="product-description">
                      ...
                      <form id="geniusformdata" action="{{route('admin-childcat-update',[ $storename,$data->id ] )}}" method="POST" enctype="multipart/form-data">
                        {{csrf_field()}}
                          ...
                          <div class="col-lg-7">
                              <select id="cat" required="">
                                  <option value="">{{ __('Select Category') }}</option>
                                    @foreach($cats as $cat)
                                      <option data-href="{{ route('admin-subcat-load',[$storename,$cat->id]) }}" value="{{ $cat->id }}" {{ $cat->id == $data->subcategory->category->id ? "selected":"" }}>{{ $cat->name }}</option>
                                    @endforeach
                                </select>
                          </div>
                        </div>
