							...
							<div class="row">
								<div class="col-lg-12">
										...
										<ul class="links">
											...
											<li>
												<a href="{{ route('admin-vendor-index',$storename) }}">{{ __("Vendors List") }}</a>
											</li>
										</ul>
								</div>
							</div>
								...
								<div class="heading-area">
	                                ...
	                                <div class="action-list">
	                                    <select class="process select1 vdroplinks {{ $gs->reg_vendor == 1 ? 'drop-success' : 'drop-danger' }}">
	                                      <option data-val="1" value="{{route('admin-gs-regvendor',[$storename,1])}}" {{ $gs->reg_vendor == 1 ? 'selected' : '' }}>{{ __("Activated") }}</option>
	                                      <option data-val="0" value="{{route('admin-gs-regvendor',[$storename,0])}}" {{ $gs->reg_vendor == 0 ? 'selected' : '' }}>{{ __("Deactivated") }}</option>
	                                    </select>
	                                  </div>
								</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-vendor-datatables',$storename) }}',
               columns: [
