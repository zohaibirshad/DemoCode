                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                        ...
                                        <ul class="links">
                                            ...
                                            <li>
                                                <a href="{{ route('admin-vendor-index',$storename) }}">{{ __("Vendors") }}</a>
                                            </li>
                                            <li>
                                                <a href="{{ route('admin-vendor-show',[$storename,$data->id]) }}">{{ __("Details") }}</a>
                                            </li>
                                        </ul>
                                </div>
                            </div>
                                                        ...
                                                        <div class="table-responsive">
                                                                <table id="example2" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        <tr>
                                                                        <td><a href="{{ route('front.product', [$storename,$dt->slug]) }}" target="_blank">{{ sprintf("%'.08d",$dt->id) }}</a></td>
                                                                            <td>{{ $dt->type }}</td>
                                                                            @php 
                                                                            ...
                                                                            @endphp
                                                                            <td>{{ $stck  }}</td>
                                                                            <td>{{  App\Models\Product::convertPrice($storename,$dt->price) }}</td>
                                                                            <td>
                                                                                <div class="action-list">
                                                                                <select class="process select droplinks {{ $dt->status == 1 ? 'drop-success' : 'drop-danger' }}">
                                                                                    <option data-val="1" value="{{ route('admin-prod-status',[$storename,'id1' => $data->id, 'id2' => 1]) }}" {{ $dt->status == 1 ? 'selected' : '' }}>{{ __("Activated") }}</option>
                                                                                    <<option data-val="0" value="{{ route('admin-prod-status',[$storename,'id1' => $data->id, 'id2' => 0]) }}" {{ $dt->status == 0 ? 'selected' : '' }}>{{ __("Deactivated") }}</option>
                                                                                </select>
                                                                                </div>
                                                                            </td>
                                                                            <td>
                                                                                <a href=" {{ route('admin-prod-edit',[$storename,$dt->id]) }}" class="view-details">
	...
	// Gallery Section Update
		...
		$(document).on("click", ".set-gallery1" , function(){
			...
			$('.selected-image .row').html('');
				$.ajax({
						type: "GET",
						url:"{{ route('admin-vr-show',$storename) }}",
						data:{id:pid},
