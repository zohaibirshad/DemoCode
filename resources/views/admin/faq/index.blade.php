							...
							<div class="row">
								<div class="col-lg-12">
										...
										<ul class="links">
											...
											<li>
												<a href="{{ route('admin-faq-index',$storename) }}">{{ __('Faq') }}</a>
											</li>
										</ul>
								</div>
							</div>
								...
								<div class="heading-area">
	                                ...
	                                <div class="action-list">
	                                    <select class="process select droplinks {{ $gs->is_faq == 1 ? 'drop-success' : 'drop-danger' }}">
	                                      <option data-val="1" value="{{route('admin-gs-isfaq',[$storename,1])}}" {{ $gs->is_faq == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
	                                      <option data-val="0" value="{{route('admin-gs-isfaq',[$storename,0])}}" {{ $gs->is_faq == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
	                                    </select>
	                                  </div>
								</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-faq-datatables',$storename) }}',
               columns: [
        ...
        $(".btn-area").append('<div class="col-sm-4 text-right">'+
        	'<a class="add-btn" data-href="{{route('admin-faq-create',$storename)}}" id="add-data" data-toggle="modal" data-target="#modal1">'+
