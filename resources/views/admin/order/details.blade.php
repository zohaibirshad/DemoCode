                                ...
                                <div class="col-lg-6">
                                    <div class="special-box">
                                        ...
                                        <div class="footer-area">
                                            <a href="{{ route('admin-order-invoice',[$storename,$order->id]) }}" class="mybtn1"><i class="fas fa-eye"></i> {{ __('View Invoice') }}</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-lg-6">
                                ...
                                @foreach($cart->items as $key => $product)
                                    <tr>
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                @if(isset($user))
                                                <a target="_blank" href="{{route('admin-vendor-show',[$user->id,$storename])}}">{{$user->shop_name}}</a>
                                                @else
                                                {{ __('Vendor Removed') }}
                                                @endif
                                                @else 
                                            ...
                                            <td>
                                                ...
                                                @endphp
                                                @if(isset($user))
                                              <a target="_blank" href="{{ route('front.product',[ $product['item']['slug'],$storename]) }}">{{mb_strlen($product['item']['name'],'utf-8') > 30 ? mb_substr($product['item']['name'],0,30,'utf-8').'...' : $product['item']['name']}}</a>
                                                @else
                                                <a target="_blank" href="{{ route('front.product',[ $product['item']['slug'] ,$storename ]) }}">{{mb_strlen($product['item']['name'],'utf-8') > 30 ? mb_substr($product['item']['name'],0,30,'utf-8').'...' : $product['item']['name']}}</a>
                                                @endif
                                                @else 
        ...
        <h4 class="modal-title d-inline-block">{{ __('License Key') }}</h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                ...
                <div class="modal-body">
                    ...
                    <form method="POST" action="{{route('admin-order-license',[$order->id,$storename])}}" id="edit-license" style="display: none;">
                        {{csrf_field()}}
