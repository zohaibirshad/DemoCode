							...
							<div class="row">
								<div class="col-lg-12">
										...
										<ul class="links">
											...
											<li>
												<a href="{{ route('admin-comment-index',$storename) }}">{{ __('Comments') }}</a>
											</li>
										</ul>
								</div>
							</div>
								...
								<div class="heading-area">
	                                ...
	                                <div class="action-list">
	                                    <select class="process select droplinks {{ $gs->is_comment == 1 ? 'drop-success' : 'drop-danger' }}">
	                                      <option data-val="1" value="{{route('admin-gs-iscomment',[$storename,1])}}" {{ $gs->is_comment == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
	                                      <option data-val="0" value="{{route('admin-gs-iscomment',[$storename,0])}}" {{ $gs->is_comment == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
	                                    </select>
	                                  </div>
								</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-comment-datatables',$storename) }}',
               columns: [
