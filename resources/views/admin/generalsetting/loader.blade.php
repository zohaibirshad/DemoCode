              ...
              <div class="row">
                <div class="col-lg-12">
                    ...
                    <ul class="links">
                      ...
                      <li>
                        <a href="{{ route('admin-gs-load',$storename) }}">{{ __('Website Loader') }}</a>
                      </li>
                    </ul>
                </div>
              </div>
              ...
              <div class="row justify-content-center">
                <div class="col-lg-6">
                    <div class="special-box">
                        ...
                        <form class="uplogo-form" id="geniusform" action="{{ route('admin-gs-update',$storename) }}" method="POST" enctype="multipart/form-data">
                          {{csrf_field()}}   
                                ...
                                {{ __('Loader') }} :
                                      ...
                                      <div class="action-list">
                                          <select class="process select droplinks {{ $gs->is_loader == 1 ? 'drop-success' : 'drop-danger' }}">
                                            <option data-val="1" value="{{route('admin-gs-isloader',[$storename,1])}}" {{ $gs->is_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                                            <option data-val="0" value="{{route('admin-gs-isloader',[$storename,0])}}" {{ $gs->is_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                                          </select>
                                        </div>
                          </div>
                                ...
                                {{ __('Loader') }} :
                                      ...
                                      <div class="action-list">
                                          <select class="process select droplinks {{ $gs->is_admin_loader == 1 ? 'drop-success' : 'drop-danger' }}">
                                            <option data-val="1" value="{{ route('admin-gs-is-admin-loader',[$storename,1]) }}" {{ $gs->is_admin_loader == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                                            <option data-val="0" value="{{ route('admin-gs-is-admin-loader',[$storename,0]) }}" {{ $gs->is_admin_loader == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                                          </select>
                                        </div>
                          </div>
