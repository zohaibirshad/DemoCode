							...
							<div class="row">
								<div class="col-lg-12">
										...
										<ul class="links">
											...
											<li>
												<a href="{{ route('admin-report-index',$storename) }}">{{ __('Reports') }}</a>
											</li>
										</ul>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="heading-area">
	                                ...
	                                <div class="action-list">
	                                    <select class="process select droplinks {{ $gs->is_report == 1 ? 'drop-success' : 'drop-danger' }}">
	                                      <option data-val="1" value="{{route('admin-gs-isreport',[$storename,1])}}" {{ $gs->is_report == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
	                                      <option data-val="0" value="{{route('admin-gs-isreport',[$storename,0])}}" {{ $gs->is_report == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
	                                    </select>
	                                  </div>
								</div>
...
{{-- DATA TABLE --}}
    ...
    <script type="text/javascript">
		...
		var table = $('#geniustable').DataTable({
               ...
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-report-datatables',$storename) }}',
               columns: [
