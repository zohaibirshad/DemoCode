    ...
    <div class="row">
      <div class="col-lg-12">
        <ul class="pages">
          ...
          <li>
            <a href="{{ route('front.cart',$storename) }}">
              {{ $langg->lang121 }}
            </a>
          </li>
        </ul>
                    ...
                    <tr class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
                      <td class="product-img">
                        <div class="item">
                          ...
                          <p class="name"><a href="{{ route('front.product', [$storename,$product['item']['slug'] ]) }}">{{mb_strlen($product['item']['name'],'utf-8') > 35 ? mb_substr($product['item']['name'],0,35,'utf-8').'...' : $product['item']['name']}}</a></p>
                        </div>
                      </td>
                                            <td>
                      ...
                      <td class="unit-price quantity">
                        <p class="product-unit-price">
                          {{ App\Models\Product::convertPrice($storename,$product['item']['price']) }}                        
                        </p>
        ...
        @endif
                      ...
                      <td class="total-price">
                        <p id="prc{{$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values'])}}">
                          {{ App\Models\Product::convertPrice($storename,$product['price']) }}                 
                        </p>
                      </td>
                      <td>
                        <span class="removecart cart-remove" data-class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}" data-href="{{ route('product.cart.remove',[$storename,$product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) ]) }}"><i class="icofont-ui-delete"></i> </span>
                      </td>
                    </tr>
                    @endforeach
                    @endif
          ...
          <div class="order-box">
            ...
            <ul class="order-list">
              <li>
                ...
                <P>
                  <b class="cart-total">{{ Session::has('cart') ? App\Models\Product::convertPrice($storename,$totalPrice) : '0.00' }}</b>
                </P>
              </li>
              <li>
                <p>
                ...
                <P>
                  <b class="discount">{{ App\Models\Product::convertPrice($storename,0)}}</b>
                  <input type="hidden" id="d-val" value="{{ App\Models\Product::convertPrice($storename,0)}}">
                </P>
              </li>
              <li>
                <p>
            ...
            <div class="total-price">
              ...
              <p>
                  <span class="main-total">{{ Session::has('cart') ? App\Models\Product::convertPrice($storename,$mainTotal) : '0.00' }}</span>
              </p>
            </div>
            <div class="cupon-box">
              <div id="coupon-link">
              ...
              <form id="coupon-form" class="coupon">
                ...
                <input type="hidden" class="coupon-total" id="grandtotal" value="{{ Session::has('cart') ? App\Models\Product::convertPrice($storename,$mainTotal) : '0.00' }}">
                <button type="submit">{{ $langg->lang134 }}</button>
              </form>
            </div>
            <a href="{{ route('front.checkout',$storename) }}" class="order-btn">
              {{ $langg->lang135 }}
            </a>
          </div>
        </div>
