...
@section('content')
						...
						<div class="content-area">
							<div class="mr-breadcrumb">
								<div class="row">
									<div class="col-lg-12">
											<h4 class="heading"> {{ $langg->lang704 }} <a class="add-btn" href="{{ url()->previous() }}"><i class="fas fa-arrow-left"></i> {{ $langg->lang550 }}</a></h4>
											<ul class="links">
												<li>
													<a href="{{ route('admin.dashboard',$storename) }}">{{ $langg->lang441 }} </a>
												</li>
											<li>
												<a href="javascript:;">{{ $langg->lang447 }} </a>
											</li>
											<li>
												<a href="{{ route('vendor-import-index',$storename) }}">{{ $langg->lang449 }}</a>
											</li>
												<li>
								...
								<div class="row">
									<div class="col-lg-12">
										<div class="product-description">
					                      ...
					                      <form id="geniusform" action="{{route('vendor-import-update',[$storename,$data->id])}}" method="POST" enctype="multipart/form-data">
					                        {{csrf_field()}}
                        ...
                        @include('includes.admin.form-both')  
												...
												<div class="row">
													...
													<div class="col-lg-7">
														<input type="text" class="input-field" placeholder="{{ $langg->lang707 }}" name="affiliate_link" required="" value="{{ $data->affiliate_link }}">
                                              ...
                                              @foreach($cats as $cat)
                                                  <option data-href="{{ route('vendor-subcat-load',[$storename,$cat->id]) }}" value="{{$cat->id}}" {{$cat->id == $data->category_id ? "selected":""}} >{{$cat->name}}</option>
                                              @endforeach
						                                     </select>
													</div>
												</div>
												...
												<div class="row">
													...
													<div class="col-lg-7">
	                                                  ...
	                                                  <option data-href="{{ route('admin-childcat-load',[$storename,$sub->id]) }}" value="{{$sub->id}}" >{{$sub->name}}</option>
	                                                  @endforeach
	                                                  @else
	                                                  @foreach($data->category->subs as $sub)
	                                                  <option data-href="{{ route('admin-childcat-load',[$storename,$sub->id]) }}" value="{{$sub->id}}" {{$sub->id == $data->subcategory_id ? "selected":""}} >{{$sub->name}}</option>
	                                                  @endforeach                                                  
	                                                  @endif
												...
												<div class="row">
													...
													<div class="col-lg-7">
															<select id="imageSource" name="image_source">
                                                  				<option value="file" {{ !filter_var($data->photo,FILTER_VALIDATE_URL) ? 'selected' : '' }}>{{ $langg->lang710 }}</option>
                                                  				<option value="link" {{ filter_var($data->photo,FILTER_VALIDATE_URL) ? 'selected' : '' }}>{{ $langg->lang711  }}</option>
															</select>
													</div>
												</div>
													...
													<div class="col-lg-7">
													  <div class="img-upload">
														  <div id="image-preview" class="img-preview" style="background: url({{ $data->photo ? asset('assets/images/products/'.$data->photo):asset('assets/images/noimage.png') }});">
															  ...
															  <input type="file" name="photo" class="img-upload" id="image-upload" {{ filter_var($data->photo,FILTER_VALIDATE_URL) ? 'required' : '' }}>
															</div>
...
// Gallery Section Update
    ...
    $(document).on("click", ".set-gallery" , function(){
        ...
        $('.selected-image .row').html('');
            $.ajax({
                    type: "GET",
                    url:"{{ route('vendor-gallery-show',$storename) }}",
                    data:{id:pid},
  ...
  $(document).on('click', '.remove-img' ,function() {
    ...
    $(this).parent().parent().remove();
	    $.ajax({
	        type: "GET",
	        url:"{{ route('vendor-gallery-delete',$storename) }}",
	        data:{id:id}
	    });
  });
  ...
  $(document).on('submit', '#form-gallery' ,function() {
		  $.ajax({
		   url:"{{ route('vendor-gallery-store',$storename) }}",
		   method:"POST",
...
$('.cropme').click();
 ...
 setTimeout(
    function() {
      ...
      $.ajax({
        url: "{{route('vendor-prod-upload-update',[$storename,$data->id])}}",
        type: "POST",
        data: {"image":img},
        success: function (data) {
          if (data.status) {
