...
@section('content')
						...
						<div class="content-area">
							<div class="mr-breadcrumb">
								<div class="row">
									<div class="col-lg-12">
											<h4 class="heading"> {{ $langg->lang704 }}<a class="add-btn" href="{{ route('vendor-prod-index',$storename) }}"><i class="fas fa-arrow-left"></i> {{ $langg->lang550 }}</a></h4>
											<ul class="links">
                      <li>
                        <a href="{{ route('vendor-dashboard',$storename) }}">{{ $langg->lang441 }}</a>
                      </li>
                      <li>
                        <a href="javascript:;">{{ $langg->lang444 }} </a>
                      </li>
												<li>
													<a href="javascript:;">{{ $langg->lang630 }}</a>
												</li>
												<li>
													<a href="{{ route('vendor-prod-edit',[$storename,$data->id]) }}">{{ $langg->lang705 }}</a>
												</li>
											</ul>
									</div>
								</div>
								...
								<div class="row">
									<div class="col-lg-12">
										<div class="product-description">
					                      ...
					                      <form id="geniusform" action="{{route('vendor-prod-update',[$storename,$data->id])}}" method="POST" enctype="multipart/form-data">
					                        {{csrf_field()}}
                                              ...
                                              @endforeach
												...
												<div class="row">
													...
													<div class="col-lg-7">
	                                                  ...
	                                                  <option data-href="{{ route('vendor-childcat-load',[$storename,$sub->id]) }}" value="{{$sub->id}}" >{{$sub->name}}</option>
	                                                  @endforeach
	                                                  @else
	                                                  @foreach($data->category->subs as $sub)
	                                                  <option data-href="{{ route('vendor-childcat-load',[$storename,$sub->id]) }}" value="{{$sub->id}}" {{$sub->id == $data->subcategory_id ? "selected":""}} >{{$sub->name}}</option>
	                                                  @endforeach                                                  
	                                                  @endif
...
// Gallery Section Update
    ...
    $(document).on("click", ".set-gallery" , function(){
        ...
        $('.selected-image .row').html('');
            $.ajax({
                    type: "GET",
                    url:"{{ route('admin-gallery-show',$storename) }}",
                    data:{id:pid},
  ...
  $(document).on('click', '.remove-img' ,function() {
    ...
    $(this).parent().parent().remove();
	    $.ajax({
	        type: "GET",
	        url:"{{ route('admin-gallery-delete',$storename) }}",
	        data:{id:id}
	    });
  });
  ...
  $(document).on('submit', '#form-gallery' ,function() {
		  $.ajax({
		   url:"{{ route('admin-gallery-store',$storename) }}",
		   method:"POST",
