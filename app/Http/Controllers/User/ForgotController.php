...
class ForgotController extends Controller
{
    ...
    {
      return view('user.forgot',compact('storename'));
    }
    ...
    public function forgot(Request $request,$storename)
    {
      ...
      $input =  $request->all();
      if (User::where('storename',$storename)->where('email', '=', $request->email)->count() > 0) {
      // user found
      $admin = User::where('storename',$storename)->where('email', '=', $request->email)->firstOrFail();
