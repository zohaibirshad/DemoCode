...
class PaytmController extends Controller
{
    ...
    $user = Auth::user();
     ...
     $notify_url = action('User\PaytmController@notify',$storename);
    ...
    $gs = Generalsetting::where('storename',$storename)->first();
		...
		$paramList["CALLBACK_URL"] = route('user.paytm.notify',$storename);
	 ...
	 */
	function getConfigPaytmSettings($storename) {
    $gs = Generalsetting::where('storename',$storename)->first();
	...
	public function notify( Request $request,$storename ) {
        ...
        $order = UserSubscription::where('storename',$storename)->where('user_id','=',Session::get('item_number'))
            ->orderBy('created_at','desc')->first();
            $order->delete();
            return redirect(route('user.payment.cancle',$storename));
		}
    }
}
