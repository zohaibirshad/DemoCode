...
class RazorpayController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $rdata = Generalsetting::where('storename',$storename)->first();
 ...
 public function store(Request $request,$storename){
    ...
    $user = Auth::user();
     ...
     $return_url = action('User\PaypalController@payreturn',$storename);
     $cancel_url = action('User\PaypalController@paycancle',$storename);
     $notify_url = action('User\RazorpayController@notify',$storename);
    ...
    // Redirect to paypal IPN
        ...
        return view( 'front.razorpay-checkout', compact( 'data','displayCurrency','json','notify_url','storename') );
...
$order = UserSubscription::where('storename',$storename)->where('user_id','=',Session::get('item_number'))
            ->orderBy('created_at','desc')->first();

        $user = User::findOrFail($order->user_id);
        $package = $user->subscribes()->where('status',1)->orderBy('id','desc')->first();
        $subs = Subscription::where('storename',$storename)->findOrFail($order->subscription_id);
        $settings = Generalsetting::where('storename',$storename)->first();
        ...
        return redirect()->route('user-dashboard',$storename)->with('success','Vendor Account Activated Successfully');
            ...
            $razorpayOrder = $this->api->order->fetch(session('razorpay_order_id'));
            $order_id = $razorpayOrder['receipt'];
        $payment = UserSubscription::where('storename',$storename)->where('user_id','=',$order_id)
            ->orderBy('created_at','desc')->first();
        $payment->delete();
