...
class ImportController extends Controller
{
    ...
    {
         $user = Auth::user();
         $datas = $user->products()->where('product_type','affiliate')->where('storename',$storename)->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('action', function(Product $data) use ($storename){
                                return '<div class="action-list"><a href="' . route('vendor-import-edit',[$storename,$data->id]) . '"> <i class="fas fa-edit"></i>'.$this->vendor_language->lang715.'</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> '.$this->vendor_language->lang716.'</a><a href="javascript:;" data-href="' . route('vendor-prod-delete',[$storename,$data->id]) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            }) 
    ...
    {
        return view('vendor.productimport.index',compact('storename'));
    }
    ...
    public function createImport($storename)
    {
        $cats = Category::all();
        $sign = Currency::where([['is_default','=',1],['storename',$storename]])->first();
        return view('vendor.productimport.createone',compact('cats','sign','storename'));
    }
    ...
    public function importCSV($storename)
    {
        $cats = Category::all();
        $sign = Currency::where([['is_default','=',1],['storename',$storename]])->first();
        return view('vendor.productimport.importcsv',compact('cats','sign','storename'));
    }
    ...
    public function store(Request $request,$storename)
    {
        ...
        //--- Redirect Section        
        $msg = 'New Affiliate Product Added Successfully.<a href="'.route('vendor-import-index',$storename).'">View Product Lists.</a>';
    ...
    public function edit($storename,$id)
    {
        ...
        $data = Product::findOrFail($id);
        $sign = Currency::where([['is_default','=',1],['storename',$storename]])->first();
        return view('vendor.productimport.editone',compact('cats','data','sign','storename'));
    }
    ...
    public function update(Request $request,$storename,$id)
    {
        ...
        if($request->image_source == 'file'){
            ...
            {
                $this->validate($request,[
                    'file'       => 'sku' => 'min:8|unique:products,sku,'.$id
        ...
        //--- Redirect Section        
        $msg = 'Product Updated Successfully.<a href="'.route('vendor-import-index',$storename).'">View Product Lists.</a>';
        \Session::put('success',$msg);
            return redirect()->back();     
        //--- Redirect Section Ends    
    }
