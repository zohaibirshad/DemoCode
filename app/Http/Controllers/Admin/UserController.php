...
class UserController extends Controller
{
    public function __construct()
        ...
        {
             $datas = User::where('storename',$storename)->orderBy('id')->get();
             ...
             return Datatables::of($datas)
                                ->addColumn('action', function(User $data) use ($storename){
                                    ...
                                    '<option data-val="0" value="'. route('admin-user-ban',[$storename,'id1' => $data->id, 'id2' => 1]).'" '.$s.'>Block</option>'.
                                    '<option data-val="1" value="'. route('admin-user-ban',[$storename,'id1' => $data->id, 'id2' => 0]).'" '.$ns.'>UnBlock</option></select>';
                                    return '<div class="action-list"><a href="' . route('admin-user-show',[$storename,$data->id]) . '" > <i class="fas fa-eye"></i> Details</a><a data-href="' . route('admin-user-edit',[$storename,$data->id]) . '" class="edit" data-toggle="modal" data-target="#modal1"> <i class="fas fa-edit"></i>Edit</a><a href="javascript:;" class="send" data-email="'. $data->email .'" data-toggle="modal" data-target="#vendorform"><i class="fas fa-envelope"></i> Send</a>'.$ban.'<a href="javascript:;" data-href="' . route('admin-user-delete',[$storename,$data->id]) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                                }) 
                                ->rawColumns(['action'])
        ...
        {
            return view('admin.user.index',compact('storename'));
        }

        //*** GET Request
        public function image($storename)
        {  
        ...
        public function show($storename,$id)
        {
            ...
            return view('admin.user.show',compact('data','storename'));
        }

        //*** GET Request
        public function ban($storename,$id1,$id2)
        {
        ...
        {
            $data = User::findOrFail($id);
            return view('admin.user.edit',compact('data','storename'));
        }
...
// OTHER SECTION ENDS
        ...
        {
             $datas = Withdraw::where('storename','=','user')->where('type','=','user')->orderBy('id','desc')->get();
             //--- Integrating This Collection Into Datatables
             return Datatables::of($datas)
                                ...
                                ->addColumn('action', function(Withdraw $data) {
                                    $action = '<div class="action-list"><a data-href="' . route('admin-withdraw-show',[$storename,$data->id]) . '" class="view details-width" data-toggle="modal" data-target="#modal1"> <i class="fas fa-eye"></i> Details</a>';
                                    if($data->status == "pending") {
                                    $action .= '<a data-href="' . route('admin-withdraw-accept',[$storename,$data->id]) . '" data-toggle="modal" data-target="#confirm-delete"> <i class="fas fa-check"></i> Accept</a><a data-href="' . route('admin-withdraw-reject',$data->id) . '" data-toggle="modal" data-target="#confirm-delete1"> <i class="fas fa-trash-alt"></i> Reject</a>';
                                    }
                                    $action .= '</div>';
                                    return $action;
                                }) 
        ...
        {
            return view('admin.user.withdraws',compact('storename'));
        }

        //*** GET Request       
        public function withdrawdetails($id,$storename)
        {
            $sign = Currency::where('storename',$storename)->where('is_default','=',1)->first();
            $withdraw = Withdraw::findOrFail($id);
            return view('admin.user.withdraw-details',compact('withdraw','sign','storename'));
        }
