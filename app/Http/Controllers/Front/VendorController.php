...
class VendorController extends Controller
{
    ...
    public function index(Request $request,$storename,$slug)
    {
        ...
        dd($storename.$string);
        $vendor = User::where('storename',$storename)->where('shop_name','=',$string)->firstOrFail();

        $data['vendor'] = $vendor;
        $data['services'] = DB::table('services')->where('storename',$storename)->where('user_id','=',$vendor->id)->get();
        ...
        $prods = Product::when($minprice, function($query, $minprice) {
                                    ...
                                    ->when(empty($sort), function ($query, $sort) {
                                        return $query->orderBy('id', 'DESC');
                                    })->where('storename',$storename)->where('status', 1)->where('user_id', $vendor->id)->get();

        $vprods = (new Collection(Product::filterProducts($prods)))->paginate(9);
        $data['vprods'] = $vprods;


        return view('front.vendor',compact('storename'),$data);
    }

    //Send email to user
    public function vendorcontact(Request $request,$storename)
    {
        $user = User::where('storename',$storename)->findOrFail($request->user_id);
        $vendor = User::where('storename',$storename)->findOrFail($request->vendor_id);
    ...
    $conv = Conversation::where('storename',$storename)->where('sent_user','=',$user->id)->where('subject','=',$subject)->first();
        if(isset($conv)){
            $msg = new Message();
