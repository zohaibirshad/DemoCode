...
{
    public function loadpayment($storename,$slug1,$slug2)
    {
        ...
        $gateway = '';
        if($pay_id != 0) {
            $gateway = PaymentGateway::where('storename',$storename)->findOrFail($pay_id);
        }
        
        return view('load.payment',compact('payment','pay_id','gateway','curr','storename'));
    }

    public function checkout($storename)
    {
        // dd('sdvsd1');
        $this->code_image();
        if (!Session::has('cart')) {
            return redirect()->route('front.cart',$storename)->with('success',"You don't have any product to checkout.");
        }
...
// If a user is Authenticated then there is no problm user can go for checkout
        ...
        {
                $gateways =  PaymentGateway::where('storename',$storename)->where('status','=',1)->get();
                $pickups = Pickup::where('storename',$storename)->get();
                ...
                {                        
                    ...
                    if(count($users) == 1)
                    {
                        ...
                        $shipping_data  = DB::table('shippings')->where('storename',$storename)->where('user_id','=',$users[0])->get();
                        if(count($shipping_data) == 0){
                            $shipping_data  = DB::table('shippings')->where('storename',$storename)->where('user_id','=',0)->get();
                        }
                        else{
                ...
                if($gs->multiple_packaging == 1)
                {
                    ...
                    if(count($users) == 1)
                    {
                        $package_data  = DB::table('packages')->where('storename',$storename)->where('user_id','=',$users[0])->get();
                        if(count($package_data) == 0){
                            $package_data  = DB::table('packages')->where('storename',$storename)->where('user_id','=',0)->get();
                        }
                        else{
...
// If guest checkout is activated then user can go for checkout
        ...
        return view('front.checkout', ['storename'=>$storename,'products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'checked' => $ck, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);  
                ...
                // dd($shipping_data.$package_data.$vendor_shipping_id.$vendor_packing_id);
        return view('front.checkout', ['storename'=>$storename,'products' => $cart->items, 'totalPrice' => $total, 'pickups' => $pickups, 'totalQty' => $cart->totalQty, 'gateways' => $gateways, 'shipping_cost' => 0, 'digital' => $dp, 'curr' => $curr,'shipping_data' => $shipping_data,'package_data' => $package_data, 'vendor_shipping_id' => $vendor_shipping_id, 'vendor_packing_id' => $vendor_packing_id]);                 
               }
...
// If guest checkout is Deactivated then display pop up form with proper error message
    ...
    public function cashondelivery(Request $request,$storename)
    {
        ...
        if(!empty($prod['item']['license']) && !empty($prod['item']['license_qty']))
        {
                foreach($prod['item']['license_qty']as $ttl => $dtl)
                {
                    ...
                    {
                        $dtl--;
                        $produc = Product::where('storename',$storename)->findOrFail($prod['item']['id']);
        ...
        $order = new Order;
        $success_url = action('Front\PaymentController@payreturn',$storename);
        ...
        $order['storename'] = $storename;
            if (Session::has('affilate')) 
            {
                ...
                $user = User::where('storename',$storename)->findOrFail(Session::get('affilate'));
        ...
        $notification->save();
                    ...
                    {
                       $coupon = Coupon::where('storename',$storename)->findOrFail($request->coupon_id);
                       $coupon->used++;
        ...
        foreach($cart->items as $prod)
        {
            ...
            if(!empty($x))
            {
                $product = Product::where('storename',$storename)->findOrFail($prod['item']['id']);
                $x = (int)$x;
                $x = $x - $prod['qty'];
    ...
    public function gateway(Request $request,$storename)
    {
